...
console.log(this.TrackTargetPos);
	...
	CheckClick:function()
		{
			...
			if(Mouse.x > this.x-this.size/2*WindowScale && Mouse.x<this.x+this.size/2*WindowScale &&
				Mouse.y > this.y-this.size/2*WindowScale && Mouse.y<this.y+this.size/2*WindowScale &&
				Mouse.clicked && this.life>0
				)
				{
					if(this.target == c)
					{
						Mouse.clicked=false;
						this.target.LastTarget=this;
						//c.stance = c.atackImage;
						//console.log(c.arguments[0]);
						for(var i=1;i<=4;i++)
						{
							if(this.target.CurrentAbility==i)
							{
								this.target.skills[i-1](
	...
	draw:function()
	{
		...
		ctx.drawImage(GorillaSps,this.size*this.hMod,this.size*this.vMod,this.size,this.size,this.size*WindowScale/-2,this.size*WindowScale/-2,this.size*WindowScale,this.size*WindowScale);
		ctx.restore();
		//ctx.fillStyle="#000000";
	...
	update:function()
	{
		...
		if(this.life<this.maxlife/2){this.phase=2;}
		this.colidePlayer();
		this.CheckClick();
