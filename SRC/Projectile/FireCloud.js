function FireCloud(x, y,size,speed,angle,range,targetArray)
{
...
FireCloud.prototype = Object.create(Projectile.prototype);



FireCloud.prototype.update = function() {
	if(this.life==this.maxLife)
		this.explode();
	this.life--;
};

FireCloud.prototype.draw = function() {
	ctx.beginPath();
...
FireCloud.prototype.explode = function(){
	...
	for(var i=0;i<this.targetArray.length;i++)
			if(Distance(this.x,this.targetArray[i].x,this.y,this.targetArray[i].y)<this.range)
			{
				...
				if(!ObjInArray(this.targetArray[i].buffArray,Burn))
					this.targetArray[i].buffArray.push(new Burn(this.targetArray[i],50));
			}
